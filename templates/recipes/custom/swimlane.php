...
.swimlane .callout-box p.action-button{ text-align:left;}
...
$cell1 .= '<div class="content small-24 medium-14 large-16 columns">'."\n";
$cell1 .= '<h1>'.$callout->post_title.'</h1>'."\n";
$cell1 .= apply_filters('the_content', $callout->post_content);

if (!empty(get_post_meta($callout->ID, 'destination', true))) {
	$cell1 .= '<p class="action-button"><a href="'.get_post_meta($callout->ID, 'destination', true).'" class="button flat small">'.get_post_meta($callout->ID, 'action_text', true).'</a></p>'."\n";
}
$cell1 .= '</div>'."\n";

$cell2 .= '<div class="image small-24 medium-8 large-6 columns">'."\n";
